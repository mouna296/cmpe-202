...
const DateSelector = ({ selectedDate, setSelectedDate }) => {
			...
			<button
				...
				className={`flex min-w-[48px] flex-col items-center justify-center rounded p-1 font-semibold ${
					isThisDate
						...
						: isToday
						? 'bg-gradient-to-br from-slate-100 to-white ring-2 ring-inset ring-indigo-800 hover:from-white hover:to-white'
						: isPast(date)
						? 'bg-gradient-to-br from-gray-600 to-gray-500 text-white hover:from-gray-500 hover:to-gray-400'
						: 'bg-to-white hover:from-white hover:to-white'
				}`}
				onClick={() => {
	...
	return (
		<div className="flex flex-col gap-2">
			<div className="relative flex items-stretch justify-between gap-2 rounded-md bg-gradient-to-br from-slate-800 to-slate-700 p-2 font-semibold text-white">
				{auth.role === 'admin' || !isPast(new Date().setDate(selectedDate.getDate() - 1)) ? (
					<button
						title="Go to yesterday"
						className={'rounded hover:bg-gradient-to-br hover:from-lime-600 hover:to-lime-600'}
				...
				{isEditing ? (
					...
					<div
						className="flex w-full items-center justify-center rounded text-center text-xl hover:bg-lime-600  sm:text-2xl"
						onClick={() => {
							SetIsEditing(true)
						}}
					>
				...
				<div className="flex items-center justify-between gap-2">
					...
					<button
						title="Go to today"
						className="rounded px-1 hover:bg-gradient-to-br hover:from-lime-600 hover:to-lime-600"
						onClick={handleToday}
					>
